...
export function processStreamContent(
  ...
  if (state.isBufferingToolCall) {
    ...
    // Only emit when [/function_calls] is complete to avoid duplicate emission
    const hasCompleteBlock = state.contentBuffer.includes('[/function_calls]')
    if (toolCalls.length > 0 && (hasCompleteBlock || !state.contentBuffer.includes('[function_calls]') )) {
