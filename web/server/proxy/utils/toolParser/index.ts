...
export function parseToolCallsStream(
  ...
  if (state.isBufferingToolCall) {
    ...
    // Only emit when we have a complete [/function_calls] block
    const hasCompleteBlock = state.contentBuffer.includes('[/function_calls]')
    console.log('[StreamHandler] toolCalls:', toolCalls.length, 'hasComplete:', hasCompleteBlock, 'hasEmitted:', state.hasEmittedToolCall, 'bufLen:', state.contentBuffer.length)
    if (toolCalls.length > 0 && hasCompleteBlock) {
...
export function flushToolCallBuffer(
  ...
  if (toolCalls.length > 0 && !state.hasEmittedToolCall) {
    for (const tc of toolCalls) {
      tc.index = state.toolCallIndex++
      delete tc.rawText
      result.push({
