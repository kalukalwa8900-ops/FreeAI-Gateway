...
export class PerplexityAdapter {
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{ stream: Readable; sessionId: string }> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const axiosResp = await axios.post(QUERY_ENDPOINT, JSON.stringify(data), { headers, responseType: 'stream', timeout: 120000 })
        const statusCode = axiosResp.status
        if (statusCode === 403) { stream.emit('error', new Error('Cloudflare challenge detected')); reject(new Error('Cloudflare challenge detected')); return }
        if (statusCode === 429) { stream.emit('error', new Error('Rate limit exceeded')); reject(new Error('Rate limit exceeded')); return }
        axiosResp.data.on('data', (chunk: Buffer) => { stream.push(chunk); chunks.push(Buffer.from(chunk)) })
        axiosResp.data.on('end', () => { stream.push(null) })
        axiosResp.data.on('error', (error: Error) => { stream.emit('error', error) })
        const sessionId = sessionData?.backend_uuid || requestId
        resolve({ stream, sessionId })
      } catch(error: any) { stream.emit('error', error); reject(error) }
      // legacy compat
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    ...
    try {
      ...
      return axios.delete(deleteUrl, { headers, data: requestBody, timeout: 30000 })
        .then(resp => { sessionCache.delete(cacheKey); return resp.status >= 200 && resp.status < 300 })
        .catch(err => { console.error('[Perplexity] Delete request error:', err); sessionCache.delete(cacheKey); return false })
    } catch (error) {
