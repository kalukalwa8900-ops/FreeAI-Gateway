...
export class GLMAdapter {
  ...
  private async acquireToken(): Promise<string> {
    ...
    if (refresh_token !== refreshToken) {
      ...
      await store.updateAccount(this.account.id, {
...
export class GLMStreamHandler {
  ...
  async handleNonStream(stream: any): Promise<any> {
    // 通过收集流式 SSE chunks 来获取完整内容（包括图片等）
            ...
            if (delta.tool_calls) {
              delta.tool_calls.forEach((tc: any) => {
                ...
                if (!toolCalls[idx]) toolCalls[idx] = { ...tc }
                  ...
                  toolCalls[idx].function = toolCalls[idx].function || {}
      ...
      stream.once('close', () => {
        ...
        resolve({
          ...
          choices: [{
            ...
            message: {
              role: 'assistant',
              content: validToolCalls.length > 0 ? null : (fullContent || null),
