...
import { toolsToSystemPrompt, hasToolPromptInjected } from '../utils/tools'
...
const X_FE_VERSION = 'prod-fe-1.0.279'
...
export class ZaiAdapter {
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{ response: AxiosResponse; chatId: string; requestId: string }> {
    ...
    if (request.tools && request.tools.length > 0) {
      const toolPromptExists = processedMessages.some((m: any) => 
        typeof m.content === 'string' && hasToolPromptInjected(m.content)
      )
      if (!toolPromptExists) {
    ...
    const features = {
      image_generation: request.image_generation === true || false,
      preview_mode: true,
      flags: [] as string[],
      ...
      onEvent: (event: any) => {
        try {
          ...
          if (result.phase === 'answer' && result.delta_content) {
            ...
            // Handle agent-generated images (async, fire-and-forget before safeEnd)
            const versionInfo = result.metadata?.version_info
            const agentImageFiles = (versionInfo?.success && versionInfo?.metadata?.new_files?.length > 0)
              ? versionInfo.metadata.new_files.filter((f: any) => f.path.match(/\.(png|jpg|jpeg|gif|webp)$/i))
              : []
            const agentSnapshotId = versionInfo?.message_id || ''
            ...
            if (agentImageFiles.length > 0 && agentSnapshotId && agentToken) {
              Promise.all(agentImageFiles.map(async (fileInfo: any) => {
                try {
                  const encodedPath = encodeURIComponent(fileInfo.path)
                  const imgUrl = `${ZAI_API_BASE}/api/v1/storage/versions/${agentSnapshotId}/file?path=${encodedPath}`
                  ...
                  const mimeType = fileInfo.path.endsWith('.png') ? 'image/png' : 'image/jpeg'
                  const imgMarkdown = `\n![image](data:${mimeType};base64,${b64})\n`
                  const imgChunk = { ...createBaseChunk(this.chatId, this.model, this.created), choices: [{ index: 0, delta: { content: imgMarkdown }, finish_reason: null }] }
                  transStream.write(`data: ${JSON.stringify(imgChunk)}\n\n`)
                  console.log('[Z.ai] Injected agent image:', fileInfo.path)
                } catch (e) {
                  console.error('[Z.ai] Failed to download agent image:', e)
                }
              })).finally(() => doFinish())
            } else {
              doFinish()
