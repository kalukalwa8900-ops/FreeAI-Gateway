...
export function Dashboard() {
  ...
  const statusDot = (ok: boolean) => ok
    ? <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_#10b981]"></span><span className="text-[10px] t-sub">运行中</span></span>
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <h2 className="text-3xl font-light tracking-tight t-heading font-headline">全局概览</h2>
          <p className="t-sub text-sm">监控网关实时流量与服务健康状态</p>
        </div>
        <button
          onClick={refresh}
          disabled={isLoading}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 t-sub hover:t-heading hover:bg-white/10 transition-all text-sm"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="glass-card p-6 border border-white/5 relative overflow-hidden group">
          ...
          <p className="t-sub text-[10px] font-bold uppercase tracking-[0.2em] mb-1">总请求数</p>
          <p className="text-3xl font-light t-heading tracking-tighter">{totalRequests.toLocaleString()}</p>
        </div>
        ...
        <div className="glass-card p-6 border border-white/5 relative overflow-hidden group">
          ...
          <p className="t-sub text-[10px] font-bold uppercase tracking-[0.2em] mb-1">成功率</p>
          <p className="text-3xl font-light t-heading tracking-tighter">{successRate.toFixed(1)}%</p>
        </div>
        ...
        <div className="glass-card p-6 border border-white/5 relative overflow-hidden group">
          ...
          <p className="t-sub text-[10px] font-bold uppercase tracking-[0.2em] mb-1">平均延迟</p>
          <p className="text-3xl font-light t-heading tracking-tighter">{avgLatency > 0 ? `${avgLatency}ms` : '—'}</p>
        </div>
        ...
        <div className="glass-card p-6 border border-white/5 relative overflow-hidden group">
          ...
          <p className="t-sub text-[10px] font-bold uppercase tracking-[0.2em] mb-1">活跃供应商</p>
          <p className="text-3xl font-light t-heading tracking-tighter">{activeProviders}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass-card rounded-3xl p-6 border border-white/5">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-medium t-heading">供应商状态</h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {providers && providers.length > 0 ? providers.slice(0, 6).map((p: any) => (
                <div key={p.id} className="bg-av-surface-high/40 backdrop-blur-[30px] p-4 rounded-2xl border border-white/5 hover:bg-white/10 transition-all cursor-pointer">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center text-xs font-bold t-heading">
                      {(p.name || '?').slice(0, 2).toUpperCase()}
                    </div>
                    <span className="text-sm font-medium t-heading truncate">{p.name}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    {statusDot(p.status !== 'error')}
                    <span className="text-[10px] t-hint">{p.requestCount ?? 0} 请求</span>
                  </div>
                </div>
              )) : (
                <div className="col-span-3 text-center t-hint text-sm py-8">暂无供应商数据</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="glass-card rounded-3xl p-6 border border-white/5 h-full">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-medium t-heading">最近活动</h3>
            </div>
            <div className="space-y-6">
                  ...
                  <div key={item.id || i} className="flex gap-4 group">
                    ...
                    <div className="flex flex-col flex-1 pb-6">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-medium t-heading truncate max-w-[140px]">{item.title || item.modelName || item.providerName || '请求记录'}</span>
                        <span className="text-[10px] t-hint shrink-0 ml-2">{timeStr}</span>
                      </div>
                      <p className="text-xs t-sub">{item.description || (item.providerName ? `供应商: ${item.providerName}` : '')}</p>
                    </div>
                  </div>
                )
              }) : (
                <p className="t-hint text-sm text-center py-8">暂无最近活动</p>
              )}
            </div>

            {/* 系统报告卡片 */}
            <div className="mt-6 p-4 rounded-2xl bg-gradient-to-br from-cyan-400/20 to-violet-500/10 border border-white/5">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-xs font-bold t-heading">代理状态</h4>
              ...
              <div className="flex gap-2 mt-4">
                ...
                <button
                  ...
                  disabled={!proxyRunning || proxyLoading}
                  className="flex-1 py-2 bg-white/10 disabled:opacity-40 disabled:cursor-not-allowed t-heading text-[10px] font-bold uppercase tracking-widest rounded-xl hover:bg-white/20 transition-all"
                >
                  停止
                </button>
              </div>
              <button
                ...
                className="w-full mt-2 py-1.5 border border-white/10 t-sub text-[10px] font-bold uppercase tracking-widest rounded-xl hover:border-cyan-400/30 hover:text-cyan-400 transition-all"
              >
                代理设置
              </button>
            </div>
